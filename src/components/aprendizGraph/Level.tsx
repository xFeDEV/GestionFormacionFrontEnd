...
import { grupoService, DashboardFilters, DistribucionNivel } from "../../api/grupo.service"; // 1. Importar servicio e interfaces
...
export default function LevelAprendices({ filters }: LevelGroupProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const data: DistribucionNivel[] = await grupoService.getDistribucionPorNivel(filters);

        // Transformar los datos para el formato que necesita el gráfico de barras
        const categories = data.map(item => item.nivel);
        const seriesData = data.map(item => item.total_aprendices_formacion);
        ...
        console.error("Error al cargar datos de nivel:", err);
